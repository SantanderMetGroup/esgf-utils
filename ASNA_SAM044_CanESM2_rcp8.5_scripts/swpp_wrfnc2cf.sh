...
function get_filelist_yearmon(){
  ...
  #grep -E "wrf${filetype}_d01_${yearmon}.*\.nc" cache.filelist | sort  # Asi no pilla bien los ficheros xtrm
  #grep -E "wrf${filetype}_d01_*${yearmon}*\.nc" cache.filelist | sort
}
...
wxajcmd="${wrfncxnj} -a ${xnjattr} -g ${geofile} --fullfile ${fullfile} --temp-dir ${tmpdir}/xnj" 
...
# Comentado porque no hay anios bisiestos en esta simulacion y sino no posprocesa los febrero de dichos anios
...
#  fi
  ...
  ${wxajcmd} \
    ...
    --time-units="Days since 1949-12-01T00:00:00" \
